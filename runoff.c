...
bool vote(int voters, int rank, string name)
{
    for (int k = 0; k < candidate_count; k++)
    {
        if (!strcmp(candidates[k].name, name))
        {
            preferences[voters][rank] = k;
            printf("Voter number:%i,Rank:%i, Voted: %s \n", voters, rank, name);
            printf("preferencias %i\n", preferences[voters][rank]);
            return true;
        }
